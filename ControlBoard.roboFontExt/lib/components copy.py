        ...
        if info["name"] == self.name:
            ...
            if "state" in info:
                ...
                if info["state"] == "on":
                    ...
                    colorValue = [v*100 for v in colorValue]
                    self._red.component.brightness(colorValue[0])
                    self._green.component.brightness(colorValue[1])
                    self._blue.component.brightness(colorValue[2])
...
class ComponentButton(BSer.Button):
    ...
    def downCallback(self):
        ...
        postEvent("ControlBoardInput", name=self.name, state="down", type=self.type)
        
    def upCallback(self):
        # Button was released
        postEvent("ControlBoardInput", name=self.name, state="up", type=self.type)
        
    def holdCallback(self):
        # Button was held for at least 1 second
        postEvent("ControlBoardInput", name=self.name, state="hold", type=self.type)
...
class ComponentRotaryEncoder(BSer.RotaryEncoder):
    ...
    def cwCallback(self):
        ...
        postEvent("ControlBoardInput", name=self.name, state="changed", value="cw", type=self.type)
        
    def ccwCallback(self):
        # Encoder was rotated counter-clockwise
        postEvent("ControlBoardInput", name=self.name, state="changed", value="ccw", type=self.type)
